...
describe('GET task by ID', () => {
  it('should GET a task by ID', (done) => {
    const taskId = 1
    chai.request(server).get('/api/task/' + taskId).end((err, response) => {
      response.should.have.status(200)
...
describe('POST /api/tasks', () => {
  ...
  it('should NOT POST a new task without the name property', (done) => {
    ...
    chai.request(server).post('/api/tasks').send(task).end((err, response) => {
      ...
      response.text.should.be.eq('The name should ve at least 3 characters long')
    })
    done()
  })
})
...
describe('PATCH /api/tasks/:id', () => {
  it('should PATCH an existing tasks', (done) => {
    const taskId = 1
    const task = {
      name: 'Task 1 patch'
    }
  ...
  it('should NOT PATCH an existing tasks with a name that is less than 3 characters long', (done) => {
    const taskId = 1
    const task = {
      name: "T",
    }
